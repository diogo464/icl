...
class Visitor implements AstVisitor {
	...
	public Visitor() {
		...
		this.method = this.main_class.visitMethod(Opcodes.ACC_PUBLIC + Opcodes.ACC_STATIC, "main",
				"([Ljava/lang/String;)V", null,
				null);
		this.method.visitCode();
		this.method.visitMaxs(256, 256);
		this.method.visitFieldInsn(Opcodes.GETSTATIC, "java/lang/System", "out", "Ljava/io/PrintStream;");
	}

	public List<JvmCompiler.CompiledClass> finish() {
		var compiled_classes = new ArrayList<JvmCompiler.CompiledClass>();
		this.method.visitMethodInsn(Opcodes.INVOKESTATIC, "java/lang/String", "valueOf", "(I)Ljava/lang/String;",
				false);
		this.method.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/io/PrintStream", "println",
	...
	public void acceptDef(AstDef node) {
		...
		if (parent_frame != null) {
			...
			this.method.visitFieldInsn(Opcodes.PUTFIELD, stackframe.getTypename(), StackFrame.PARENT_VARNAME,
	...
	private void compilePushStackFrame(int depth) {
		if (depth > this.environment.getCurrentDepth())
			throw new IllegalStateException("Attempt to load stackframe at depth greater the current maximum depth");
		...
		while (current_depth != depth) {
			...
			this.method.visitFieldInsn(Opcodes.GETFIELD, current_frame.getTypename(), StackFrame.PARENT_VARNAME,
