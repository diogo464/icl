...
class CompilerVisitor implements AstVisitor {
    ...
    private final JvmEnv environment;
    ...
    public CompilerVisitor() {
        ...
        this.environment = new JvmEnv(this.name_generator);
    ...
    public void acceptDef(AstDef node) {
        ...
        var stackframe = this.environment.pushFrame(varcount);

        var stackframe_name = this.name_generator.generateStackframeName();
        var writer = new ClassWriter(0);
        writer.visit(Opcodes.V1_5, Opcodes.ACC_PUBLIC, stackframe_name, null,
                "java/lang/Object", null);
        for (var decl : node.decls) {
            writer.visitField(Opcodes.ACC_PUBLIC, this.name_generator.generateVariableName(), "I", null, null);
        }
        ...
        create_descriptor += ")L" + stackframe_name + ";";

        writer.visitMethod(Opcodes.ACC_STATIC + Opcodes.ACC_PUBLIC, "create", create_descriptor, null, null);
    }
